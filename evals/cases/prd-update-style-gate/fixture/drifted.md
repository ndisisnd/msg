...
- **Auth constraint** — the quiet-hours window is stored per user account and only readable in a session authenticated as that user.
- **Platform constraint** — web only (Chrome 110+, Safari 16+); no native client this release.
...
- Does the window follow the user-profile timezone or the browser timezone when the two disagree? Owner: Staff PM · Due: before eng kickoff.
- Reminders held across the window are delivered as one digest, confirmed with design. Owner: Staff PM · Due: N/A.
