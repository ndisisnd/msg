...
description: merge Step 1/2 — verify branch protection via script-branch-protection.sh before any merge, policy-conditional per branch_protection mode (enforced refuses / optional warns + proceeds / skip doesn't verify). The machine-enforced half of the v2 safety floor.
---
...
merge is now a policy decision (`../shared/refs/policy-schema-merge.md` §2): under the
...
from `devkit/policy.json` (`../shared/refs/policy-schema-merge.md` §2). **No file /
...
| `NO_REMOTE` (exit 2) | no git remote | **refuse** (`unprotected`) — merge needs a GitHub remote to merge PRs | **refuse** (`unprotected`) — same, regardless of mode |
...
```bash
bash .claude/scripts/script-branch-protection.sh --bootstrap
```
...
Merge never sets protection itself — it only verifies. Bootstrapping is an
