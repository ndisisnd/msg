---
name: merge-init
description: merge --init spec — detect ship capability + policy (release-flow topology, branch protection, deploy/smoke CLIs, PLATFORMS.md gaps), interview per gap, install OSS-first, and complete devkit/policy.json. Never merges, deploys, opens PRs, or writes PLATFORMS.md.
type: reference
---

# `/merge --init` — ship-capability + policy setup

`--init` is a **setup mode**, not a ship. Merge's tooling is less about test
...
This ref covers the
**merge-specific detection**. The policy file's schema, status vocabulary, validation
...
**without a follow-up `/merge --init`** leaves **no `staging_ready` record** —
...
`/merge --init` to derive readiness, and the warn note names it.
...
| `UNPROTECTED <b> <missing>` (exit 1), protection **available** | offer `script-branch-protection.sh --bootstrap` (gated, per-item); decline → record `optional` or `skip` with a `reason` |
...
these modes into refuse / warn-proceed / skip at merge time lives in `policy-schema-merge.md`
...
- `ga:true`/absent — unchanged: note the gap and point the user at **`/pre-merge --init`**, which
 ...
 Merge `--init` **never writes `steps.ci`** and **never
...
declarations stay owned by `/msg --init`; merge `--init` is read-only to that file.
...
- *web* — `no `.env.staging` on disk (declared in `staging_config`) and
  ...
  `devkit/PLATFORMS.md`, then re-run `/merge --init`.*
...
(`../../shared/refs/policy-schema-merge.md` §5) — a resolved **fact**, re-derived on every
...
When `release_flow.mode:"direct"`, there is no staging branch: `/merge --staging`
**refuses** with `no_staging_stage` (naming both `/merge --production` and
...
- `staging_ready` — per-platform staging-readiness fact (item 6; **`staged` flow
  ...
  `--staging` (`../../shared/refs/policy-schema-merge.md` §5).
- flips **`init:true`** on completion, stamps `generated` + `generated_by:"merge --init"`.
...
`installed`), and the full field spec are authoritative in `policy-schema-merge.md`; do not
duplicate them here.
