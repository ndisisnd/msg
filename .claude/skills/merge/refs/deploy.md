...
Merge deploys using the commands declared per platform in
...
2. **Cell markers arrive normalised**: blank, `—`, and `[USER: …]` are all the empty string — the "empty" path below. Merge never invents a command.
...
4. `release_model` arrives with its provenance — `release_model_source=declared` or `inferred` plus the matching `WARN`, never silently. The rule (including why **a macOS row declares its model**) is stated once in `SKILL.md` § *Release model* and `../shared/refs/policy-schema-merge.md` §4; surface every `WARN` in the run report.
