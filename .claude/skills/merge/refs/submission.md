...
processing and review that merge neither owns nor blocks on. This ref
specifies the whole model: the primitive (submitted-not-live, backend-health
smoke) **and** the lifecycle merge represents and hands off.
...
path (`../shared/refs/policy-schema-merge.md` §4). Nothing below is
...
Release under `submission` moves through four states. Merge **owns** exactly
...
|---|---|---|
| **submit** | **merge owns** | the deploy-cmd runs; exit 0 = the artifact was accepted onto its target track |
...
Merge owns **through submit-accepted**. The moment the store takes custody,
the lifecycle is out-of-band: merge records the state it last observed
...
Merge records the **target track** with the outcome and reports the platform
...
**review** rejection is out-of-band — merge never sees it (it was handed off).

Worked shapes, from the fixture store outputs (`evals/fixtures/merge/`):
...
the build artifact** — never the released app. Merge runs it and **labels its
...
Because merge stops at submit-accepted, its final act for a `submission`
...
app is live, which would require store-status polling merge deliberately does
...
`handed_off`**. In v4 merge always emits `handed_off` — it does not poll.
