...
- **Check prior PRDs before drafting** — scan `features/prd-*/` for overlap with the intake row's idea. Surface every match by ID before populating sections; never silently re-spec a problem an existing PRD already covers.
