...
release; (b) if the commits are genuinely un-signed-off, run **`/post-merge
...
```bash
S=.claude/scripts/stamp-prd.sh; bash "$S" <prd-path> staging-signoff "<today>@<STAGING_HEAD>"
```
...
Per `refs/protection.md` + `../shared/refs/policy-schema-post-merge.md` §2:
resolve `mode_main`, run `post-merge-protection.sh --verify main`, and read the
...
| 5 | `held` + `STALE=true` | refuse `release_in_flight` (stale variant) and print the script's `UNLOCK_CMD` verbatim — the human decides; post-merge never auto-steals a lock |
...
non-increasing build number, so post-merge stops rather than pushing a doomed
...
capture logs. A deploy failure emits a `post-merge` finding
...
1. **Stamp `status: done`** via the shared scalar writer — the `eng → done`
   ...
   ```bash
   S=.claude/scripts/stamp-prd.sh; bash "$S" <prd-path> status done
   ```
