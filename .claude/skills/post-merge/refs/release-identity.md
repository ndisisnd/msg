...
description: The release identity contract for post-merge --production — version, build number, provenance, tag. The version source of truth is the last v* tag reachable on prod (READ-ONLY — post-merge never writes a VERSION file or a bump commit). script-release-identity.sh resolves the whole set; this ref is the contract it implements and the rules the model applies to its verdicts.
---
...
decisive, not incidental: post-merge is forbidden from modifying source
...
file**, so tagging is consistent with the floor. post-merge **reads** tags to
...
post-merge ships PRDs (features), so the default bump is **minor**. From `0.0.0`
...
Because the comparison uses the **post-merge** count, it can only trip when the
...
- `PROVENANCE=asserted_unverified` — no probe declared: post-merge deployed from the merged prod branch it controls, recorded with a note, never a fail. Declaring a probe upgrades the assertion to a verified check.
...
`fail` — post-merge cuts the annotated tag `NEXT_TAG` on the prod release commit
and pushes it (`refs/production.md` Step 9). Both the tag target and the build are
**post-merge** values, relying on the Step-5 fetch; without it the tag would point
...
- No store-number probing — build numbers come from git history, and the monotonicity check compares against post-merge's own last tag, not a store query.
