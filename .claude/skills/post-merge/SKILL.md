...
description: >
  ...
  deploy → verify per release model — smoke the live target, or submission-accepted
  for store apps → emit a human test script → stamp staging sign-off on approval)
  and `--production` (double-confirmed staging→main release
...
invocation. (In `release_flow=direct` the staging *stage* is absent, so its
sign-off is **inactive — not waived** (there is nothing to sign off, § *Release
flow*); the human judgment it carried is preserved by the **inline human-test
approval** defined in `refs/production.md` § *Inline human-test approval*, which
...
- Does NOT run a second `--production` while one is in flight — the **release lock** (a remote `release-lock-<prod>` git tag, `refs/production.md` § *Release lock*) refuses `release_in_flight`, naming the holder (who/when/sha); it releases on every exit of the acquiring run (success, failed ship, or refusal-after-acquire) and a stale lock (> 2h) prints the manual unlock rather than dead-ending (`refs/refusal-patterns.md`, `../shared/refs/policy-schema-post-merge.md` §6). A `--staging` merge during an in-flight production ship refuses the same way (it would advance `staging` past the certified window). Uncontended acquire/release is silent — solo single-run and `direct` flow feel no friction.
- Does NOT run when `post-merge-protection.sh --verify` reports the branch unprotected **and** the `branch_protection` policy resolves to `enforced` (the default / no-file case) — refuses with the bootstrap instruction; `optional` warns + proceeds, `skip` doesn't verify (`../shared/refs/policy-schema-post-merge.md` §2). `NO_GH`/`NO_REMOTE` refuse regardless of mode.
- Does NOT run `--staging` into a staging environment `--init` recorded as **unready** (a platform with `gaps[]` in `staging_ready`) **when `staging_readiness` resolves to `enforced`** (the default) — refuses `staging_unready`, naming the exact missing artifact + fix; `optional` warns + proceeds, `skip` doesn't guard, and a **missing** record (pre-C9 init) only warns, never refuses (`../shared/refs/policy-schema-post-merge.md` §5, `refs/staging.md`).
- Does NOT modify source code. **This is the canonical, complete enumeration of its sanctioned writes** (`../shared/refs/safety-floor.md` and `refs/release-identity.md` defer here — they do not restate it):
  1. the **two PR merges** (`--staging` feature→`stg`; `--production` `<head>`→`prod`);
  2. the **`staging-signoff:` frontmatter stamp** — written by `--staging` Step 7 on approval, **and re-stamped by `--production` Step 1** when an unpinned legacy stamp is confirmed (`<today>@<STAGING_HEAD>`);
  3. the **`INTAKE.md` `status: completed` stamp** on each shipped PRD's mapped row (`--production`, v3 D14);
  4. the **`status: done` frontmatter stamp** on each shipped PRD (`--production` Step 10, on a successful ship) — the terminal `eng → done` transition, filed beside the intake stamp; the existing `reviewed:` and `staging-signoff:` stamps stay intact (24-prd-lifecycle-lanes);
  ...
  7. the transient **release-lock tag** `release-lock-<prod>` pushed on acquire and deleted on release around a `--production` ship (`refs/production.md` § *Release lock*, C8);
  8. the **move of a shipped PRD's folder** `features/wip/prd-<n>-<slug>/ → features/done/prd-<n>-<slug>/` (`--production` Step 10, on a successful ship) — a whole-directory rename carrying the PRD plus its colocated `reports/`/`preflight.md`/`test/` (24-prd-lifecycle-lanes). `features/` is gitignored, so this is a plain `mv` (`git mv` only on a legacy repo that still tracks its PRDs).
  Items 6–7 are **metadata on a commit** — they write **no tracked file**, so the safety floor holds; the version source of truth is the tag, never a VERSION file or bump commit (D8). Item 8 **relocates docs/metadata** — it renames the PRD's own directory (a directory rename, not a file edit) and writes **no source code** (`src/` is untouched), so the safety floor holds there too.
...
| Out | staging_signoff | `staging-signoff: <YYYY-MM-DD>@<certified sha>` stamped into PRD frontmatter (`--staging`, on approval) — the sha is the staging commit that was deployed and human-tested. Also **re-stamped by `--production` Step 1** (`<today>@<STAGING_HEAD>`) when a human confirms an unpinned legacy stamp |
...
opens PRs, or deploys (`refs/protocol-init.md`).
...
|---|---|---|
| `staged` (default / no file) | merge feature→`stg` (Steps 1–7 below, unchanged) | PR `stg`→`prod` (Steps 1–10 below) |
...
(`../shared/refs/policy-schema-pre-merge.md` § `components[]`), so today the staging-scoped
...
(`../shared/refs/policy-schema-post-merge.md` §4). It is **orthogonal to `release_flow`**:
...
| 4 | **Open release PR** — `gh pr create --base $PROD --head <head>` where `<head>` is **flow-dependent**: `$STG` in `staged` flow, the resolved **feature branch** in `direct` flow (never hardcoded `staging` — F1); release-style body: PRDs, linked reports, per-platform rollback notes from `PLATFORMS.md` `rollback_possible` (iOS flagged `IRREVERSIBLE`). No resolvable PRD → refuse `no_prd` (release the lock first) | `refs/production.md` |
...
| 10 | **PRD lifecycle → `done`** — only on success (same condition as Steps 8–9, provenance not `fail`): for each shipped PRD, stamp its frontmatter `status:` to `done` via the shared `stamp-prd.sh` writer (the terminal `eng → done` transition — the existing `reviewed:` / `staging-signoff:` stamps stay untouched) **and** move its whole folder `features/wip/prd-<n>-<slug>/ → features/done/prd-<n>-<slug>/` (carrying `reports/`/`preflight.md`/`test/`; plain `mv` — `features/` is gitignored). Lane-agnostic + idempotent: resolve the PRD across `features/{planned,wip,done}/prd-<n>-*/` (and legacy flat `features/prd-<n>-*/`), move the first hit to `done/`, skip with a note if already there. `--staging` never runs this; a failed ship neither stamps nor moves (24-prd-lifecycle-lanes) | `refs/production.md` |
...
5. **Hand off to `../shared/refs/fix-loop.md`** — it runs Offer #1 (plan the
   ...
   release still has a broken commit to fix forward. **For `--production`, the release
   lock releases at ship-terminal — after step 1's rollback offer and *before* this
   handoff — so the (potentially long) fix loop never holds the lock** (`refs/production.md`
   § *Release lock*, exit rows 9–11).
...
- `refs/protocol-init.md` — `--init` mode (protection policy, deploy/smoke CLIs, PLATFORMS.md gap delegation; no merge/PR/deploy)
...
- `../shared/refs/policy-schema.md` — the shared core of the `devkit/policy.json` schema + read-contract (§0 `init` lifecycle, §1 `release_flow`, §2b `github_actions`, validation rules)
- `../shared/refs/policy-schema-post-merge.md` — post-merge's half: §2 `branch_protection`, `steps.<key>` + §3, §4 `release_model`, §5 `staging_ready`, §6 the release lock. Pre-merge's half (`components[]`, `test_selection`, `source_signature`) is loaded only for the test-selection-miss read
