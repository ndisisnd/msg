...
- Does NOT modify source code. **This is the canonical, complete enumeration of its sanctioned writes** (`../shared/refs/safety-floor.md` and `refs/release-identity.md` defer here — they do not restate it):
  ...
  8. the **`git mv` of a shipped PRD's folder** `features/wip/prd-<n>-<slug>/ → features/done/prd-<n>-<slug>/` (`--production` Step 10, on a successful ship) — a whole-directory rename carrying the PRD plus its colocated `reports/`/`preflight.md`/`test/` (24-prd-lifecycle-lanes).
  Items 6–7 are **metadata on a commit** — they write **no tracked file**, so the safety floor holds; the version source of truth is the tag, never a VERSION file or bump commit (D8). Item 8 **relocates docs/metadata** — it renames the PRD's own directory (a tracked `git mv`, not a file edit) and writes **no source code** (`src/` is untouched), so the safety floor holds there too.
