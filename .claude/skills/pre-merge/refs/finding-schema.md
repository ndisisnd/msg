...
Each component subagent (the platform components, `security`/`migration`,
...
- **`rule`** is **required** and is the dedup + regression key. Populate it from the tool: the `gitleaks` rule id, semgrep check id, failing test/spec name, WCAG criterion, coverage metric, or a stable slug (`acceptance-unmet`, `oversize-commit`). Never null — aggregation dedups on `(category, file, line, rule)`, regressions on `(category, file, rule)`.
- **`source`** is the gate stage per `../../shared/refs/finding-schema.md`: `pre-merge:mechanical` (or `lint:`/`comment-scan`/`commit-cap`), `pre-merge:unit-int`, `pre-merge:regression`, `pre-merge:bucket:<name>`, `pre-merge:security` (or `secrets:<scanner>`/`sast:semgrep`/`dependency:<tool>`), `pre-merge:migration` (or `migration:static`), `pre-merge:prd-consistency`, `pre-merge:smoke`.
