---
name: integration
description: The integration component — run the integration test suite via the detected runner. Script only, no LLM. Non-zero exit is a blocker; a missing runner is never a blocker.
---

# `integration` — the integration-test component

Deterministic, **no LLM**. Runs on the post-sync branch (it always re-runs after
SYNC's merge), alongside `protocol-unit.md`'s pass. Findings conform to
...
- Environment-only failure (unreachable test DB/service dependency, container not
  ...
  false block from a broken local environment. When the diff touches this component's
  surface, that error also carries the `component-errored` finding (`../_common.md`).

## Minified invocation

`integration` is selection-capable. When the executor resolved a **minified** run,
execute `components[].run_minified` instead of `run` and record `selected`/`total`
(plus `fallback_reason` when the rule fell back to full). The rule, the tiers, and the
fallbacks live in **one place** — `../executor.md` §3c.

**The one component-specific fact:** `integration` runs `run_minified` at tier **S**
only. At tier **M** it runs full (`fallback_reason: "tier: M (widen-to-full)"`),
because past the smallest tier the residual risk is cross-module interaction — this
component's own domain — while `unit`'s file-precise selection still covers "this unit
is wrong".
...
(`mechanical`, `security`, `migration`) does, per `../severity-rubric.md`
§ *Fail-fast by component `criticality`*. Independent components finish so the verdict
aggregates the full picture.
