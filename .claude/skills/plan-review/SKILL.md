...
| Certification findings | Rows in the findings-table schema (`refs/certification.md`) | Written into the PRD's **§7 Plan review findings** section (created once, appended thereafter) |
...
**Write the §7 ledger.** Hand this run's findings to `script-ledger.py` as JSON. It owns locating or creating the **Plan review findings** section, deduping against prior rows, the monotonic `#`, and the `Clean` marker row:
...
- Product tune → `product-tuned: yes`: `S=.claude/scripts/script-prd-stamp.sh; [ -f "$S" ] || S="$HOME/.claude/scripts/script-prd-stamp.sh"; bash "$S" "$RESOLVED_PATH" product-tuned yes`
