...
5. **Full-suite gate (unit + integration).** After all feature groups are green, run the project's **unit + integration** suite and lint/typecheck once (discover the commands from `CLAUDE.md`, `devkit/ARCHITECTURE.md`, or the package manifest — e.g. `npm test`/`npm run lint`, `pytest`, `flutter test`). Unit + integration only — e2e / visual / perf / a11y / coverage belong to pre-merge. This catches breakage in sibling code the per-group runs never touched. Any new failure introduced by this agent's changes goes to Debug mode (`protocol-build-debug.md`, max 3 cycles) before committing. A pre-existing failure unrelated to the assigned rows is noted in the build summary, not fixed. No test or lint command → state that in the summary and continue.
   ...
   **Inject the identity fields and the artifact key with them** — `built_by` (this build's packet key or agent name) and `<K>`, the key the reviewer names its evidence artifact with. The reviewer writes `<prd-dir>/reports/review-prd-<N>-<K>.json` before returning (`refs/review/protocol.md` § Artifact); that file, not the returned prose, is what an orchestrator or gate later reads to prove this review happened. Use the **same `<K>` as this build's run report**, so the build and its review sit side by side under one key. On an orchestrated run the orchestrator supplies `<K>` (its packet key) — pass it through unchanged rather than deriving your own.

   On return: resolve every `blocker` and `high` (apply the reviewer's fix, or put its question to the human at Step 6) before proceeding; record `medium`/`low` findings, the one-line verdict **and the artifact path** in the build summary's **Review** line, ungating. A re-review after fixes is a judgment call, not a required round.
...
**Review:** <the reviewer's one-liner from Step 5a — `reviewed <n> files — <k> fixed, <q> questions, <c> comments added`, or "— clean"; list any recorded medium/low findings> · `<verdict>` · evidence: `<path to reports/review-prd-<N>-<K>.json>`
