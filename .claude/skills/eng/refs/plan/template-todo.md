...
A ticket with no explicit `kind` is a `"todo"`; the finding→issue-ticket projection that `eng --build test-json=` consumes lives in `refs/build/protocol-build-testjson.md`.
