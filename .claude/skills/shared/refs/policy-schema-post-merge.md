---
name: policy-schema-post-merge
description: The post-merge half of the devkit/policy.json schema — branch_protection (§2), staging_readiness, steps.<key> (+ §3), release_model (§4), staging_ready (§5), and the release lock (§6). Pre-merge never loads this file.
type: reference
---

# `devkit/policy.json` — the post-merge half

The sections **only post-merge reads**. The shared core (lifecycle, writers,
...
## `steps.<key>` — post-merge's per-step decisions
...
*writes* `steps.ci` (it detects the workflow) — post-merge reads it.
...
**`ci` — read by post-merge's green-CI check, not run as a step.** It records whether a
...
of passing vacuously. When post-merge's PR check finds an **empty** status-check set
...
resolves into a component's `criticality`). When post-merge becomes component-driven
...
the identical value, no second read path. Until then post-merge reads the PLATFORMS.md
column directly.
...
Documented here because this file is the canonical home for post-merge's
...
| **Release** | unconditional for any run that acquired it — deletes the remote tag, never dangles on a graceful exit (AC-LK2). Timing: success / refusal-after-acquire (Step 4 `no_prd`, post-acquire coverage-drift `stale_signoff`, `direct` inline human-test `human_test_declined`, Step 5 `red_ci`/`no_review`/`merge_failed`, Step 6 `nonmonotonic_build`) → at run termination; **failed ship → at ship-terminal, *before* the fix-loop handoff**, so the long fix loop never holds the lock (`post-merge/refs/production.md` § *Release lock*) |
