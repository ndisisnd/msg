...
| `evidence` | object | yes | Nested object — the canonical evidence shape. `tool` is required; `file`/`line`/`snippet` may be null/omitted for non-file-scoped findings. Buckets MAY add documented extra keys (e.g. bundle's `route`, `baseline_kb`, `current_kb`, `culprit`; e2e's `spec`). |
