...
| `test_selector` | string \| null | **additive**, sibling of `run_minified` — freeform note naming the selection mechanism the preflight script detected (`vitest related --changed`, `pytest-testmon`, `xcodebuild -testPlan Critical`, …). Audit only; nothing branches on it. `null` wherever `run_minified` is |
