...
| 24 | [prd-lifecycle-lanes](done/24-prd-lifecycle-lanes/plan.md) | Give `features/` three physical lanes — `planned`/`wip`/`done` — and move each PRD between them at its stage transitions: drafted → planned, branch cut → wip, production ship → done (which also stamps `status: done`). Init scaffolds the lanes; a lane-agnostic PRD-path resolver keeps the digest/roadmap/preflight/GUI working across lanes; existing flat PRDs migrate once. | Done |
