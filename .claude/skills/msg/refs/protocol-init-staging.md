...
description: >
  ...
  script-policy-set. Offered by /merge --init when it detects a
...
- Hand-off from `/merge --init` when it detects a direct-flow repo
...
| Branch protection | `script-branch-protection.sh --bootstrap` output lines | GitHub |
...
**Step 2 — Offer branch protection (gated).** Adding a staging stage means `/merge` will gate
on it, so offer to protect `staging` (and prod) now. One `AskUserQuestion`:

> header **Branch protection**, question "Apply branch protection to `staging` + prod now? (required for `/merge`)"
...
> - **Skip** — note `/merge` will refuse until protection is set; the user can re-run the script later.
...
TestFlight track for store-submission platforms). Hand off: run **`/merge --init`**, which
...
- `.claude/scripts/script-branch-protection.sh` — Step 2 `--bootstrap`
- `../../shared/refs/policy-schema.md` — canonical policy schema; writers table
- `../../merge/refs/protocol-init.md` — the readiness verifier this mode hands off to
