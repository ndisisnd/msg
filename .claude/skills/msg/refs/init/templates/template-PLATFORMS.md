...
| `notarize_status_cmd` | **optional (macOS `deploy` only)** — a command that prints the Apple **notarization status** (`status: Accepted` / `In Progress` / `Invalid`, the `xcrun notarytool info <id>` / `notarytool submit --wait` shape). When declared, `/post-merge` treats notarization as a **distinct verifiable step** (no longer folded invisibly into the deploy cmd — AC-MAC1), polled via the C7 poll primitive bounded by `smoke_poll` (or a single read if none — `notarytool --wait` already blocks to terminal). Terminal `Accepted` → verified; `Invalid` → `notarization-invalid` finding; still `In Progress` at the poll ceiling → `notarization-stall` finding — each **specific**, never a generic deploy failure. The async shape mirrors a `submission`'s *processing* state (`../../../../post-merge/refs/submission.md`). Blank / `[USER: …]` ⇒ notarization stays folded in the deploy cmd (today's behavior, unflagged). Leave `—` for non-macOS. |
