...
- `.claude/scripts/plan-em-exec-skeleton.py` — deterministic exec-table skeleton renderer; reads §6 + a `[{"fid","concern","agent"}]` spec on stdin and emits the complete skeleton table (Feature/Todos anchors filled, Execution steps + Files blank); consumed in Step 3 so anchor typos / row-text drift are impossible
