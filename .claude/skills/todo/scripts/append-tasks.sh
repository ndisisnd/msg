...
#                     Each task must have: status, agents, description.
...
#     starting at N+1 to the incoming tasks.
#   - Appends the new tasks to the array and writes the result atomically.
#   - Prints the absolute path of the todos file and the count appended.
#
# Requires: jq.
...
if ! jq -e 'all(.[]; has("status") and has("agents") and has("description"))' "$TASKS_FILE" >/dev/null; then
  echo "error: each task must have status, agents, and description fields" >&2
  exit 1
fi
...
# Assign sequential ids to incoming tasks, then concatenate
TMP_FILE="$(mktemp)"
...
echo "Appended $COUNT task(s) to $ABS_PATH"
