#!/usr/bin/env bash
# script-preflight-02-unit.sh — detect+normalize the `unit` check.
...
if [[ -n "$name" ]]; then
  mk_report unit 02 universal true always "$(tooling "$name")" "$cmd" blocking cheap '[]' ready "unit runner: $name" "$run_minified" "$test_selector"
else
