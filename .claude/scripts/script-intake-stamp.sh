#!/usr/bin/env bash
# script-intake-stamp.sh — deterministic row writer for the INTAKE.md backlog ledger
...
# in plan-pm (in-progress + prd), merge (completed) and the freehand
...
#
#   stamp        script-intake-stamp.sh <intake.md> <row-#> --status <value> [--prd <id>]
#                Rewrite the `status` cell (and `prd` when given) of one row.
#
#   append-row   script-intake-stamp.sh <intake.md> <row-#> --append-row
...
#
#   set-cell     script-intake-stamp.sh <intake.md> <row-#> --set-cell <cell> <value>
...
#
#   remove-row   script-intake-stamp.sh <intake.md> <row-#> --remove-row
#                Delete exactly one row. Never renumbers — the gap stays.
#
#   find-row     script-intake-stamp.sh <intake.md> - --find-row --prd <prd-id>
#                READ-ONLY row lookup by PRD id — the fixed-result half of
#                merge's Step 8 "find the row whose prd cell matches this
...
#
#   log-append   script-intake-stamp.sh <intake.md> <row-#> --log-append
...
usage() {
  cat >&2 <<'USAGE'
usage: script-intake-stamp.sh <intake.md-path> <row-#> <verb>
...
set_verb() {
  if [[ -n "$verb" ]]; then
    echo "script-intake-stamp: verbs --$verb and --$1 are mutually exclusive" >&2
    usage
    exit 2
  fi
  verb="$1"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "script-intake-stamp: unknown argument '$1'" >&2; usage; exit 2 ;;
  esac
done
...
if [[ ! -f "$file" ]]; then
  echo "script-intake-stamp: file not found: $file" >&2
  exit 2
fi
...
mktmp() {
  local dir
  dir=$(dirname "$file")
  mktemp "$dir/.script-intake-stamp.XXXXXX" || { echo "script-intake-stamp: cannot create temp file" >&2; exit 5; }
}
...
commit_tmp() {
  ...
  mv "$tmp" "$file" || { echo "script-intake-stamp: cannot write $file" >&2; exit 5; }
}
...
stamp)
  ...
  case $rc in
    ...
    1) echo "script-intake-stamp: row #$row not found in ledger $file" >&2; exit 1 ;;
    2) echo "script-intake-stamp: no ledger table found in $file" >&2; exit 1 ;;
    *) echo "script-intake-stamp: internal error (awk rc=$rc)" >&2; exit 2 ;;
  esac
  ;;
...
append-row)
  if [[ -z "$rtype" || -z "$idea" || -z "$goal" || -z "$grade" ]]; then
    echo "script-intake-stamp: --append-row needs --type, --idea, --goal and --grade" >&2
    usage; exit 2
  fi
  ...
  case $rc in
    0) commit_tmp "$tmp"; trap - EXIT; exit 0 ;;
    2) echo "script-intake-stamp: no ledger table found in $file" >&2; exit 1 ;;
    4) echo "script-intake-stamp: row #$row already exists in $file — rows are never reused" >&2; exit 4 ;;
    *) echo "script-intake-stamp: internal error (awk rc=$rc)" >&2; exit 2 ;;
  esac
  ;;
...
set-cell)
  ...
  case "$lc" in
    ""|"#"|date)
      echo "script-intake-stamp: cell '$cell' is immutable — # and date are never rewritten" >&2
      exit 3 ;;
    status|prd)
      echo "script-intake-stamp: cell '$cell' belongs to the stamp verb (--status/--prd), not --set-cell" >&2
      exit 3 ;;
    type|idea|goal|grade) ;;
    *)
      echo "script-intake-stamp: unknown cell '$cell' — allowed: type, idea, goal, grade" >&2
      exit 2 ;;
  esac
  ...
  case $rc in
    ...
    1) echo "script-intake-stamp: row #$row not found in ledger $file" >&2; exit 1 ;;
    2) echo "script-intake-stamp: no ledger table found in $file" >&2; exit 1 ;;
    5) echo "script-intake-stamp: ledger $file has no '$cell' column" >&2; exit 2 ;;
    *) echo "script-intake-stamp: internal error (awk rc=$rc)" >&2; exit 2 ;;
  esac
  ...
  rc=$?
  if [[ $rc -ne 0 ]]; then
    echo "script-intake-stamp: internal error (awk rc=$rc)" >&2
    exit 2
  fi
  commit_tmp "$tmp"
  trap - EXIT
...
# merge --production Step 8 needs "the row whose prd cell matches this
...
find-row)
  if [[ -z "$prd" ]]; then
    echo "script-intake-stamp: --find-row needs --prd <prd-id>" >&2
    usage; exit 2
  fi
  ...
  case $rc in
    0) exit 0 ;;
    1) echo "script-intake-stamp: no ledger row maps to PRD '$prd' in $file" >&2; exit 1 ;;
    2) echo "script-intake-stamp: no ledger table found in $file" >&2; exit 2 ;;
    *) echo "script-intake-stamp: internal error (awk rc=$rc)" >&2; exit 2 ;;
  esac
  ;;
...
log-append)
  case "$change" in
    modify|add|remove) ;;
    "") echo "script-intake-stamp: --log-append needs --change <modify|add|remove>" >&2; usage; exit 2 ;;
    *)  echo "script-intake-stamp: --change '$change' is not one of modify, add, remove" >&2; exit 3 ;;
  esac
  if [[ -z "$detail" ]]; then
    echo "script-intake-stamp: --log-append needs --detail <text>" >&2
    usage; exit 2
  fi
  [[ -n "$when" ]] || when=$(date +%F)
  ...
  if [[ ! -f "$logfile" ]]; then
    cat > "$logfile" <<'HEADER' || { echo "script-intake-stamp: cannot create $logfile" >&2; exit 5; }
...
HEADER
  ...
  if [[ -s "$logfile" && -n "$(tail -c1 "$logfile")" ]]; then
    printf '\n' >> "$logfile" || { echo "script-intake-stamp: cannot write $logfile" >&2; exit 5; }
  fi

  detail_esc=$(printf '%s' "$detail" | tr '\n\r' '  ' | sed -e 's/\\|/|/g' -e 's/|/\\|/g')
  printf '| %s | #%s | %s | %s |\n' "$when" "$row" "$change" "$detail_esc" >> "$logfile" \
    || { echo "script-intake-stamp: cannot write $logfile" >&2; exit 5; }
  exit 0
  ;;
