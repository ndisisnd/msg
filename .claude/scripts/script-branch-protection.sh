#!/usr/bin/env bash
# script-branch-protection.sh — branch-protection bootstrap + verify (C3 / D11).
#
...
# via `gh api`; merge Step 1 runs `--verify` and refuses when unprotected,
# and `/msg --init` offers `--bootstrap` when a GitHub remote exists.
#
# Modes:
#   script-branch-protection.sh --bootstrap                 set protection on staging + main
#   script-branch-protection.sh --bootstrap --contexts "ci/pre-merge,build"
#                                                        ...and require the named status
#                                                        checks to pass before merging
#   script-branch-protection.sh --verify                    check protection on staging + main
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --*)         echo "script-branch-protection: unknown flag '$1'" >&2; exit 2 ;;
    *)           ONLY_BRANCH="$1"; shift ;;
  esac
done

[[ -z "$MODE" ]] && { echo "script-branch-protection: pass --bootstrap or --verify" >&2; exit 2; }
