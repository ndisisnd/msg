...
# wave design — owns its own array rewrite (stamp-prd.sh is scalar-only).
#
# Usage:
#   plan-pm-deps-mirror.sh <prd.md>
#
...
if [[ -z "$file" ]]; then
  echo "usage: plan-pm-deps-mirror.sh <prd.md>" >&2
  exit 2
fi
if [[ ! -f "$file" ]]; then
  echo "plan-pm-deps-mirror: file not found: $file" >&2
  exit 2
fi

dir=$(dirname "$file")
tmp=$(mktemp "$dir/.deps-mirror.XXXXXX") || { echo "plan-pm-deps-mirror: cannot create temp file" >&2; exit 2; }
trap 'rm -f "$tmp"' EXIT
...
if [[ $rc -ne 0 ]]; then
  echo "plan-pm-deps-mirror: no YAML frontmatter block in $file" >&2
  exit 2
fi
