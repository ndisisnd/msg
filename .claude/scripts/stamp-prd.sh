#!/usr/bin/env bash
# stamp-prd.sh — deterministic scalar writer for PRD YAML frontmatter stamps.
#
...
# the improvised Bash/Edit stamps scattered across plan-em, plan-tune, and
# post-merge so the lifecycle fields are written one proven way.
#
# Usage:
#   stamp-prd.sh <prd.md> <field> <value>
#
...
if [[ " $ALLOWED " != *" $field "* ]]; then
  echo "stamp-prd: unknown field '$field'" >&2
  usage
  exit 2
fi

if [[ ! -f "$file" ]]; then
  echo "stamp-prd: file not found: $file" >&2
  exit 2
fi
...
case $rc in
  ...
  5) echo "stamp-prd: no YAML frontmatter block in $file" >&2
     exit 2 ;;
  *) echo "stamp-prd: internal error (awk rc=$rc)" >&2
     exit 2 ;;
esac
