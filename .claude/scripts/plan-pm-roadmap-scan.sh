...
#   plan-pm-roadmap-scan.sh --git              refine each `completion` bucket via
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --git)
      ...
      ;;
    *)
      echo "usage: plan-pm-roadmap-scan.sh [--exclude <prd-id>] [--git]" >&2
      exit 1
      ;;
  esac
done
...
if [[ -n "$GIT_MODE" ]]; then
  ...
  if [[ "$GIT_OK" == "1" ]]; then
    if command -v gh >/dev/null 2>&1 \
      ...
      echo "plan-pm-roadmap-scan: gh unavailable/unauthenticated — completion refined from git branches only" >&2
    fi
  else
    echo "plan-pm-roadmap-scan: git unavailable — completion left frontmatter-derived" >&2
  fi
fi
