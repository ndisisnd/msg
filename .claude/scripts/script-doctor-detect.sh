#!/usr/bin/env bash
# script-doctor-detect.sh — repo topology + branch-protection-availability probe.
# Consumed by /pre-merge --init and /merge --init to seed devkit/policy.json's
...
#     script-branch-protection.sh --verify for that; this script only answers
#     "is the protection API usable at all on this plan/repo"
#   - Creating the staging branch — that's /msg --init-staging's job
#
# Usage:    script-doctor-detect.sh [project-root]    (default: .)
