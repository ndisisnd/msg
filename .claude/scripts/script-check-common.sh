#!/usr/bin/env bash
# script-check-common.sh — shared helpers + normalized emit for the script-preflight-*.sh family.
#
# NOT a check itself: the filename is outside the `script-preflight-*.sh` glob
# (exit-check a, the executor's ingestion loop), so the family sweep skips it.
# It is *sourced* by every script-preflight-NN-<slug>.sh; running it directly is a no-op
# that exits 0.
#
# Provides:
...
# (It never matches the script-preflight-*.sh glob, so this is belt-and-braces.)
if [[ "${BASH_SOURCE[0]:-}" == "${0}" ]]; then
  echo "script-check-common.sh is a sourced library, not a check — nothing to run." >&2
  exit 0
fi
