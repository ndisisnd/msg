#!/usr/bin/env bash
# script-eng-db-touch.sh — production/data guardrail for autonomous builds.
#
# Flags a diff that touches database, data, or production-config surfaces the
# autonomous run must not change without the user's sign-off, so a hands-off
# build (eng --build, plan-em --team) can pause safely instead of shipping them.
#
# Usage:
#   script-eng-db-touch.sh [<base-ref>]     diff the current branch against <base-ref>
#                                    (default: main) and check changed paths
#   script-eng-db-touch.sh -                read a newline-separated path list from stdin
#
...
collect_paths() {
  ...
  local base="${1:-main}"
  command -v git >/dev/null 2>&1 || { echo "script-eng-db-touch: git not available" >&2; exit 2; }
