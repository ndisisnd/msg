#!/usr/bin/env bash
# script-eng-comment-scan.sh — A4 plain-English-comment scan (deterministic, zero-LLM).
#
...
#
# Usage:
#   script-eng-comment-scan.sh --staged            scan the staged diff
#   script-eng-comment-scan.sh <git-diff-range>    scan a range, e.g. HEAD~1..HEAD or main...HEAD
#
# Machine output:
...
command -v git >/dev/null 2>&1 || { echo "script-eng-comment-scan: git not available" >&2; exit 2; }
...
if [[ "${1:-}" == "--staged" ]]; then
  ...
  echo "usage: script-eng-comment-scan.sh (--staged | <git-diff-range>)" >&2
  exit 2
fi
