...
#   eng-commit-cap.sh --oversize-reason "<t>" also prints an OVERSIZE line to log
#                                             to the PRD ledger
#   eng-commit-cap.sh --message "<text>"      pair the cap with the prepared commit
#   eng-commit-cap.sh --message-file <path>   message (`-` = stdin); over cap without
...
read_message_file() {
  if [[ "$1" == "-" ]]; then
    ...
    [[ -f "$1" ]] || { echo "eng-commit-cap: no such message file '$1'" >&2; exit 2; }
    MESSAGE="$(cat "$1")"
  fi
  HAVE_MESSAGE=1
}
...
if (( loc > CAP )); then
  ...
  if (( HAVE_MESSAGE )); then
    ...
    echo "eng-commit-cap: commit changes ${loc} LOC (cap ${CAP}) but the prepared" >&2
