#!/usr/bin/env bash
# plan-tune-cert-status.sh — plan-tune certification gate checker.
#
...
#
# Usage:
#   plan-tune-cert-status.sh <prd.md> --product   # gate the product/plan wave
#   plan-tune-cert-status.sh <prd.md> --eng        # gate the eng/build wave
#
...
result="$(awk '
  ...
  /^##[[:space:]]/ {
    ...
    if (tolower(h) ~ /^plan tune findings/) { insec=1; found=1; next }
  }
  insec && /^\|/ {
    n=split($0, cells, "|")
    issep=1
