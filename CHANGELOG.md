...
- The two differences the docs lead with, because both are silent failures otherwise. **Invocation**: msg ships every skill with implicit invocation off, so under Codex nothing starts until you type `$name`. Describing what you want — which works on Claude — does nothing at all, and looks exactly like a failed install. **The commit gate**: Codex trust-gates hooks per content hash, so an untrusted CHANGELOG gate does not run and does not say it isn't running. An open gate and a live gate are indistinguishable right up to the release that ships without its entry. QUICKSTART Appendix C carries the liveness probe, including its one false negative — a probe run while `CHANGELOG.md` is already staged passes on a dead gate too, so the staging state has to be checked first or the result reported as inconclusive.
- `README.md`: the install section gains `--codex` and the reason the Codex leg is symlinks rather than a second install — two copies drift the first time one is updated, links cannot. A new FAQ entry answers the question a Codex user will actually ask ("I described what I wanted and nothing happened — is msg installed?") with the answer that it probably is.
- `QUICKSTART.md`: the verify lines gain their Codex equivalents step by step, so the guide's promise — a check after every action — holds on both harnesses. Appendix C covers invocation, the trust step, and session reload; a closing "what doesn't change" section pins the paths (`~/.claude/skills`, `.claude/msg/`) and the human gates, since the temptation a dual-harness port invites is to "port" `.claude/` to something Codex-flavoured and break both legs at once.
- `ARCHITECTURE.md`: a dual-harness layout section covering the symlink shape, the translation layer, and the two structural rules a contributor can violate without noticing — a new Claude-only tool name added to protocol text needs a `harness-map.md` row, and a new dispatch point needs the spawn-propagation preamble or its leaves inherit Claude vocabulary with no dictionary. It states plainly that the orchestration bindings are specified but not yet verified against a live Codex run, because a contributor reading an architecture doc has no other way to tell a designed claim from a tested one.
- `llms.txt`: the summary line and the `harness-map.md` entry, so a model reading the repo cold learns the translation layer exists before it starts editing protocol prose.

### [166] — v6.0.0 Phase 3: a Codex runner preset for the board, and the gate script pinned

- `.claude/skills/msg/refs/gui/server.py`: Changed — the board's Prompts button shells out to a coding CLI, and that was msg's one true hard dependency on `claude` being the binary on the machine. The runner templates are now named constants with a `--runner-codex` flag selecting the Codex preset. **The default does not move**: `claude` stays the default under *both* harnesses, because which harness is driving the board says nothing about which CLI is installed, and flipping it is exactly the tempting "fix" a dual-harness port invites. `--runner-codex` is mutually exclusive with an explicit `--runner` and fails loudly rather than silently picking a winner. `main()` was split into `build_parser()` and `resolve_runner()` so the eval can exercise the real argparse wiring instead of grepping source — a grep passes happily on a file where the default is overridden three lines later.
- The Codex preset is `codex exec --full-auto {prompt}`, one flag more than plan decision D8 specified. The Prompts button starts a detached background job with nobody at the terminal, so the runner must be non-interactive *and* allowed to write files. `codex exec` is the non-interactive subcommand but its default sandbox is read-only, so a bare `codex exec` would run, exit 0, and silently fail every edit — a worse failure than an error. `--full-auto` is the nearest equivalent to Claude's `acceptEdits`. **Unverified against a live binary**: the shape is confident, the exact flag spelling is not, and it sits behind an opt-in flag so a wrong guess costs an error message rather than a regression. Folded into the Phase 4 spike list.
- `evals/cases/c08-hook-claude-shape`, `evals/cases/c09-hook-codex-shape`: Added — assertion A5, the pair that proves `changelog-gate.py` needs no Codex-specific fork. c08 pins the full deny envelope for a Claude-shape `PreToolUse` event, including the whole reason text, since the reason *is* the mechanism — it tells the agent how to unblock — plus four allow paths, so a gate that denies everything cannot pass by accident. c09 runs three event shapes through the same script in the same repo state — Claude, Codex with its extra `model`/`turn_id` fields, and a hypothetical future Codex carrying a nested object, a null and an array — and compares stdout as bytes across shapes rather than only against a golden, so no two shapes can drift apart. Both checksum the gate script against its v5.6.5 version, because "the script is unchanged" is a claim only a checksum can honestly make.
- `evals/cases/c14-gui-default-runner`: Added — assertion A11. Reads v5.6.5's default runner out of the git tag at run time and asserts the live parsed default is byte-identical, so the constant refactor above cannot have moved it; then asserts the Codex preset is reachable only via the flag and that `protocol-gui.md` quotes the same strings the code uses, so a doc that drifts from the code fails here.
- `evals/manifests/claude-diff-allowlist.txt`: Changed — a fifth `gui` category, exact paths only. `protocol-gui.md` is listed even though it *is* protocol text a Claude session reads, which is the reason `protocol-init.md` was deliberately kept inside the net; the exclusion is justified by coverage rather than by category, since the only claim in that file which can change Claude behaviour is the runner default it quotes, and c14 pins that quote to the code.
- `.claude/skills/shared/refs/harness-map.md`: Fixed — Rule 2 claimed `merge` runs a hook-liveness preflight on the Codex leg. No such preflight exists in `merge`'s protocol, and nothing in the v6 commit sequence adds one. The rule now reads as what it actually is: an instruction to the agent reading the map, to prove the gate is alive before relying on it, with the reason the protocol text cannot know it might be untrusted (suite: 140/140).

### [165] — v6.0.0 Phase 2: `/msg --init` scaffolds both harnesses in one pass

- `.claude/skills/msg/refs/init/init.sh`: Changed — the scaffolder now emits `AGENTS.md` beside `CLAUDE.md`, plus a `.codex/` layer: `hooks.json` wiring the CHANGELOG gate, `config.toml` carrying the one non-default key msg needs (`[agents] max_depth = 2`, since msg's gate dispatch nests a level deeper than Codex allows out of the box), and the `msg-lead`/`msg-leaf` role TOMLs that stand in for the `model: opus` / `model: sonnet` tiers msg's protocols name. Whichever harness a developer opens the repo in second, they find a working project. Every write is idempotent — an existing file is skipped, never overwritten — which matters more than usual for `hooks.json`, because Codex trust-gates hooks per content hash and silently rewriting one would revoke the developer's trust and quietly stop the gate running.
- `AGENTS.md` shape (open item O4, resolved): a **pointer**, not a copy and not a symlink. A copy drifts the first time anyone edits one of the two files, and a stale memory file fails silently — the agent builds against conventions revised months ago and nothing errors; Codex also truncates an oversized project doc rather than refusing it, so a large `CLAUDE.md` copied across would be cut off mid-instruction. A symlink solves drift but materialises as a plain text file on Windows checkouts without developer mode, lets any tool appending to `AGENTS.md` write silently into `CLAUDE.md`, and leaves nowhere for the Codex-only notes. The pointer costs one extra file read per session and buys a memory file that cannot drift, cannot be truncated, and works everywhere. The emitted file states that trade-off in its own second paragraph, so a developer opening a nearly-empty `AGENTS.md` understands it within one line.
- Portable gate wiring: this repo's own `.codex/hooks.json` resolves the gate at `$(git rev-parse --show-toplevel)/.claude/scripts/changelog-gate.py`, and **that shape does not work downstream** — `install.sh` copies msg's scripts to `$HOME/.claude/scripts/` and nothing vendors a copy into a scaffolded project, so a `rev-parse`-only wiring would point at a file that does not exist and the gate would never fire. The emitted command therefore uses msg's existing house idiom: try the repo-local script first, fall back to the installed root, `cd` to the resolved root, and exit 0 if neither exists. `$HOME` appears deliberately — it is the only way to reach a script that genuinely lives outside the repo, it resolves for whoever runs it, and eval c13 asserts its presence as a named exception rather than scanning past it. Failing open when no script is found is the deliberate choice: the alternative blocks every Bash call in a project that never installed msg's scripts. The cost is that "gate absent" and "gate live" look identical from outside, which is why the emitted `AGENTS.md` tells the developer to verify liveness rather than assume it.
- `.claude/skills/msg/refs/init/init-setup.sh`: Changed — all five new artifacts joined `TARGETS`. Without that, `/msg --update` on a repo bootstrapped before msg grew a second harness reports `ALL_COMPLETE` and never gains a Codex leg.
- `evals/cases/c13-init-emits-codex-leg`: Added — runs the real `init.sh` in a throwaway fixture repo with its own `git init` and temp `HOME`, never the real repo or home. Seven assertion blocks: artifacts present and *not* symlinks (pinning the O4 decision), JSON/TOML parsing with load-bearing values printed rather than merely validated, the A6 unportable-path scan, `AGENTS.md` naming `CLAUDE.md` while duplicating none of its headings, byte-identical re-run, every artifact present in `TARGETS`, and two behavioural checks that actually execute the emitted hook command — a Codex-shaped event denies from the installed root, a bare `$HOME` allows silently.
- `evals/manifests/claude-diff-allowlist.txt`: Changed — a fourth `init` category, exact paths only and never globs. The scaffolder lives under `.claude/skills/` for packaging reasons but is not protocol text a Claude session reads; it is a template writer that runs in someone else's repo, so changing it cannot change Claude behaviour here, which is the only thing A8 defends. `protocol-init.md` is deliberately *not* listed — it is protocol text and stays inside the net. c12 now prints one `INIT` line per entry so the hole is visible in the golden rather than implied by silence, and an entry that stops being modified must be removed (suite: 137/137).
- `evals/manifests/memory-file-sites.txt`: Changed — `template-AGENTS.md` classified as a seventeenth site, emitter. It names `CLAUDE.md`, so c15's coverage check would otherwise fail it as unclassified; giving it the canonical memory-file sentence would be circular, since the file *is* the else-branch.

### [164] — v6.0.0 Phase 2: the project memory file is `CLAUDE.md`, else `AGENTS.md`

- `.claude/skills/`: Changed — five protocol files gained one sentence mapping `CLAUDE.md` onto whichever project memory file the harness actually reads. Codex reads `AGENTS.md` at the repo root before every task; Claude Code reads `CLAUDE.md`. msg's protocols name `CLAUDE.md` in sixteen files, but those mentions turn out to be two different things wearing the same name, and only one of them is broken under Codex.
- The split: **reader sites** — `eng/SKILL.md`, `eng/refs/build/protocol.md`, `eng/refs/build/protocol-packet.md`, `plan-em/refs/protocol-em.md`, `plan-pm/SKILL.md` — are protocol text telling the agent to go *read* the memory file for stack constraints and test-framework declarations before doing work. Those are wrong in a Codex-scaffolded project and got the sentence. **Emitter sites** — the init scripts and templates, `protocol-init.md`, `protocol-eng.md`, `protocol-gui.md`, and the GUI's `server.py`/`index.html` — *create or serve* `CLAUDE.md` as an artifact, and are correct exactly as they are. Editing a scaffolding template to "read whichever exists" would be meaningless when the point is to write the file, so they were left alone; emitting the Codex leg is commit C8's job. Two further files (`protocol-aha.md`, `plan-pm/refs/protocol-pm.md`) merely *refer* to the file — once inside a never-edit prohibition, once as an illustrative example — and are neither, so they were also left alone.
- `evals/manifests/memory-file-sites.txt`: Added — the classification for all sixteen files, committed as data so the reasoning above is checkable rather than remembered.
- `evals/cases/c15-memory-file-rule`: Added — exact-match, manifest-driven, no prose interpretation. Every reader path must carry the pinned sentence exactly once; every emitter and referent path must NOT carry it, so a well-meaning future sweep that adds the line wherever `CLAUDE.md` appears fails loudly instead of quietly rewriting scaffolding. It cross-checks manifest against reality in both directions, so a seventeenth `CLAUDE.md` mention introduced later fails as unclassified rather than silently defaulting to Claude-only.
- `evals/cases/c12-claude-diff-allowlist`: Changed — three of the C5 preamble files are also reader sites, so a dual-listed path's licence is now the union of both categories rather than preamble-only. The relaxation is narrow: the preamble line must still appear exactly once and nothing may be removed, and the memory loop independently holds the same file to additive-only edits (suite: 136/136).

### [163] — v6.0.0 Phase 2: Codex skill metadata, implicit invocation off

- `.claude/skills/*/agents/openai.yaml`: Added — one per shipping skill (`msg`, `intake`, `plan-pm`, `plan-review`, `plan-em`, `eng`, `pre-merge`, `merge`, `emulate`), carrying a `display_name` and `policy.allow_implicit_invocation: false`. Codex can activate a skill on its own when a user's prompt happens to match the skill's description, and that behaviour is on by default. msg's skills are not advisory helpers — they are explicit commands with consequences in the repo and beyond it, up to and including a production deploy. None of them should fire because a sentence sounded like a description, so every one of the nine opts out, and each file's header names its own specific consequence rather than reciting a generic warning. Claude Code never reads these files; `shared/` and `improve/` get none, being a ref library and a repo-internal tracker rather than invokable skills.
- Documented divergence: on Claude Code, describing what you want out loud can activate the right skill. Under Codex the same words do nothing until the user types `$name`. That is the deliberate cost of the trade above, and it is named in each file's header pending the README's Codex section.
- `evals/cases/c11-openai-yaml-policy`: Added — assertion A9. Enumerates skill directories from disk rather than a hardcoded list, so a tenth skill added later without metadata fails the gate instead of shipping with implicit invocation wide open. It separates the three ways the opt-out can break, including the nastiest: a value written as the *string* `"false"`, which looks correct in a diff and reads truthy at runtime.
- `evals/cases/c06-frontmatter-minimum`: Added — assertion A3. Every `SKILL.md` parses as YAML frontmatter with a non-empty `name` and `description` inside Codex's documented 1024-character cap. It prints a verdict per skill rather than the description text, so it stays green through ordinary copy edits and goes red only on a real structural break.
- Both cases parse YAML with a deliberately narrow hand-rolled reader rather than adding a `pyyaml` dependency the stock macOS `python3` does not carry, and both fail loudly on any construct the reader does not recognise instead of skipping it — an unrecognised construct is precisely what a foreign harness might also choke on (suite: 135/135).

### [162] — v6.0.0 Phase 2: thirteen preamble lines and the Claude non-regression allowlist

- `.claude/skills/`: Changed — one line added to each of nine `SKILL.md` files and four subagent entry-point refs, pointing a Codex-side reader at `shared/refs/harness-map.md` before anything else. Thirteen rather than nine because a `SKILL.md` preamble reaches only the reader who opened the skill: msg deliberately cuts spawned workers off from it (`protocol-packet.md` tells a build leaf explicitly not to read `eng/SKILL.md`, since the standalone-run paths in there are forbidden inside a packet), and orchestrators compose packet prompts that point at refs. So `eng/refs/build/protocol-packet.md`, `plan-em/refs/protocol-team.md`, `shared/refs/gate-dispatch.md` and `shared/refs/agent-watch.md` — the four places a worker starts reading having never seen a front door — carry the same line. Placement is identical everywhere: immediately after the frontmatter, one line plus one blank, twenty-six insertions and zero deletions across the thirteen.
- `evals/manifests/claude-diff-allowlist.txt`: Added — the closed list of every difference v6 is allowed to introduce under `.claude/skills/`. The v6 bet is a second harness with no forked skill content, and it only holds if the Claude side comes out behaving exactly as v5.6.5 did; the risk is that a reviewer reading a forty-file diff cannot tell additive Codex metadata from someone quietly rewording a gate. Three categories make that judgement mechanical: `preamble` (the thirteen files above), `memory` (the D9 reader sites, deliberately present-but-empty until C7 classifies them, since an empty category is a state the eval passes on rather than a guess C5 has to pre-commit to), and `new` (files absent from v5.6.5 entirely, which cannot regress Claude behaviour but still have to be expected). Listing a path licenses only its category's edit — an unrelated change riding along inside an allowlisted file still fails.
- `evals/cases/c12-claude-diff-allowlist`: Added — assertion A8, the core non-regression check. Diffs the live tree against the `v5.6.5` tag and asserts every difference falls in a manifest category, checking diff *content* rather than path membership: an allowlisted preamble file must show the canonical line plus a blank and no removed line at all. New files are counted rather than enumerated, so the verdict does not depend on commit C6's nine metadata files landing before or after (suite: 133/133, with the two known-flaky `emulate` sweep cases passing in isolation).

### [161] — v6.0.0 Phase 2: the harness map — one dictionary instead of a rewritten vocabulary

- `.claude/skills/shared/refs/harness-map.md`: Added — the single file the whole Codex port rests on. msg's protocol prose names Claude Code's tools directly and constantly (`AskUserQuestion` where it means *stop and ask the human*, `Agent` where it means *spawn a worker*, `model: opus` where it means *this one has to think hard*), and rewriting ~1,800 of those mentions into harness-neutral wording would be an enormous diff that made the text vaguer for the Claude reader who actually types those names. So the prose keeps speaking Claude and this file translates: nine binding rows covering the ask gate, `spawn_agent`, the unavailable background poll loop, the `msg-lead`/`msg-leaf` role tiers, inline skill chaining, `Monitor`, `$CLAUDE_PROJECT_DIR` → `git rev-parse --show-toplevel`, `/name` → `$name`, and `CLAUDE.md`-else-`AGENTS.md` project memory. It also states what is *not* translated and must not be: every msg path stays identical on both legs, because the Codex leg installs to the same places and reaches them through symlinks — "porting" those paths would break both legs at once.
- `harness-map.md` rules: Added — four rules the table alone cannot carry. **Spawn propagation**, because msg deliberately cuts leaves off from `SKILL.md` (`protocol-packet.md` forbids reading it) so a preamble pointing at the map never reaches a worker: under Codex every composed spawn prompt must carry the preamble line verbatim, recursively. **Hook liveness**, because Codex hooks are trust-gated per content hash and an untrusted hook does not error — it just silently stops running, which turns a changelog gate into an open door; the probe is a side-effect-free command whose text contains `git commit`, with its one false negative (a live gate also allows the probe when CHANGELOG.md is already staged) called out and guarded by checking staging state first. **Human gates do not degrade** — the ask row changes how a question is rendered, never whether it is asked or whether the run blocks, and `codex exec` refusing to run human-gated protocols is correct behaviour rather than a gap. **Skills stay explicit**, with the resulting Claude/Codex divergence named rather than left implicit.
- `harness-map.md` harness self-detection (open item O1, resolved): Added — the model's own knowledge of which harness it is in comes first; an env-fingerprint probe breaks ties; an unknown result falls back to Claude, since the Codex substitutions are only ever needed under Codex and guessing Claude is free on the Claude leg and immediately obvious on the Codex one.
- `evals/cases/c07-map-coverage`: Added — a subset test, not an equality test. For every Claude-only construct still live in protocol text, the map must carry a table row naming its Codex binding; a row for a construct that has fallen out of the prose is harmless and stays. The failure it exists to catch is one-directional drift — a construct still in use but never explained, where a Codex-side reader hits the word, finds nothing, and improvises. Plus exact-text assertions for the two rules that live outside the table (suite: 132/132).
- `evals/cases/`: Removed — thirteen empty `share-*` and `telemetry-*` case directories, leftovers from the v5.7 telemetry branch that had no `cmd` and no `expected/exit`. The runner counted and failed all thirteen, which is where this plan's "138 pre-existing cases" baseline came from; the real legacy suite is 125 cases and the tracked versions of these still live on `feat/msg-v5.7.0-telemetry`.

### [160] — v6.0.0 Phase 1: `install.sh --codex` installs the Codex lane

- `install.sh`: Added — a `--codex` flag. It runs the ordinary Claude install first, then symlinks `~/.agents/skills/<name>` at the freshly installed `~/.claude/skills/<name>` for each of the nine shipped skills, skipping `shared/` (a ref library with no SKILL.md, reached by path from the protocols) and `improve/` (repo-internal, never shipped). Symlinks rather than a second copy is the whole point: one set of bytes and one version stamp, so the two harnesses cannot drift between releases the way two installs always have. Targets are relative (`../../.claude/skills/<name>`), so the pair survives a home directory mounted at a different path. Changed — the retired-skill sweep now runs over `~/.agents/skills` as well, in two passes: names msg itself retired are removed whatever shape they are in, and links msg's own installer wrote (recognisable by pointing into `~/.claude/skills`) are removed once their name stops shipping. A directory msg never wrote, or a link pointing anywhere else, is left strictly alone — the installer does not garbage-collect another tool's skills out of a shared root. Unchanged — everything a flagless run does: no `~/.agents` path is created, read or touched without the flag, so a Claude-only install is byte-for-byte the v5.6.5 behaviour, and the single version stamp stays at `~/.claude/skills/msg/VERSION` where `/msg --version` already looks for it.
- `evals/cases/`: Added — `c01-codex-install-symlink` (fresh install under a throwaway `HOME`: nine links and only nine, each resolving into the real `~/.claude/skills` install, exactly one VERSION file in the whole home, readable through the Codex link, and stamping the version `package.json` declares, plus the flagless sub-check that `~/.agents` is never created), `c02-codex-install-idempotent` (second run exits 0 and leaves a byte-identical tree, compared as a manifest of content hashes and literal symlink targets), and `c03-codex-install-sweep` (a seeded retired directory and a seeded stale msg-written link are both removed, while a foreign skill directory and a link pointing outside `~/.claude/skills` survive) (suite: 131/131).

### [159] — v6.0.0 Phase 0: the Codex dogfooding leg — `.agents` symlinks and a `.codex` layer

- `.agents/skills/`: Added — a real committed directory holding nine relative symlinks, one per shipped skill (`msg`, `intake`, `plan-pm`, `plan-review`, `plan-em`, `eng`, `pre-merge`, `merge`, `emulate`), each pointing at `../../.claude/skills/<name>`. Codex discovers skills under `.agents/skills` and follows symlinked skill folders, so a Codex session opened in this repo sees the live skills with no sync step — the two-copy drift that has bitten every previous install simply cannot happen. Per-skill links rather than one whole-tree link, because the exclusions have to hold: `shared/` ships no SKILL.md and is reached by path from the protocols, and `improve/` is a repo-internal plan tracker that has never been part of the invokable surface.
- `.codex/config.toml`: Added — `[agents] max_threads = 6, max_depth = 2`, and nothing else. Depth 2 is the one non-default, because msg's gate dispatch nests one level deeper than Codex allows out of the box; model, provider and sandbox policy are deliberately left to the developer's own profile.
- `.codex/hooks.json`: Added — wires the existing `changelog-gate.py` as a `PreToolUse` hook on Bash, in the same schema Claude Code uses. The repo root is resolved with `git rev-parse --show-toplevel`, not `$CLAUDE_PROJECT_DIR`: that variable is set by Claude Code and by nothing else, so a hook wired through it would resolve to an empty path under Codex and the gate would silently stop firing.
- `.codex/agents/msg-lead.toml`, `.codex/agents/msg-leaf.toml`: Added — the two orchestration roles. msg's protocols name a model tier when they spawn (`model: opus` for the orchestrator, `model: sonnet` for a leaf); Codex's per-spawn model override is not durable, so on that leg the tier becomes a named role, differing in reasoning effort (high for the lead, low for the leaf). Neither role pins a `model`, consistent with msg dropping per-skill model pins in v4.
- `evals/cases/`: Added — `c05-agents-leg-derived` (nine entries, every one a symlink with the exact expected relative target, each resolved tree `diff -r`-identical to its `.claude/skills` original, `shared`/`improve` absent) and `c10-hook-wiring-portable` (no `$CLAUDE_PROJECT_DIR`, `ANTHROPIC_BASE_URL` or absolute user path anywhere under `.codex/`; hooks.json parses and wires the gate via `rev-parse`; all three TOMLs parse) (suite: 128/128).
