...
**X1 — Finding schema differs across review / test / pre-merge.** All three claim "compatible" output but define different shapes: disjoint severity enums (`fail`/`warn` vs `blocker`/`high`/`medium`/`low`), `title` vs `message`, nested vs string evidence, `rule`/`category` present in some and absent in others. This breaks pre-merge's P1 regression detection (dedup keys on a `rule` field that does not exist) and any consumer trusting the claim. **Fix:** one shared `finding-schema.md` in `shared/refs/`; conform all three; add the missing `rule`/`category` fields. Impact High, effort Medium.

**X2 — PRD frontmatter field-name drift (plan-pm / plan-em / plan-tune / plan).** plan-pm writes `product-tuned:`/`eng-tuned:`; plan-em and plan-tune read `tuned:`; `plan` writes none, because its "pick the terminal option" guidance skips every status-update branch. Result: tune gates are dead code, and a completed pipeline leaves frontmatter stale, breaking `/ship` and plan-em re-entry. **Fix:** standardize on `product-tuned:`/`eng-tuned:`; have each stage stamp its own field; document the schema once. Impact High, effort Low–Medium.

**X3 — File-location drift, bare-root vs `devkit/` (docu / msg-init / plan-pm / plan-em).** The framework stores AHA.md, ARCHITECTURE.md, GLOSSARY.md under `devkit/` and PRDs at `features/prd-[n]-[slug]/prd-[n]-[slug].md`, but several skills read/write bare-root `AHA.md`/`ARCHITECTURE.md` and unslugged `features/prd-[n]/`. docu finds none of its targets and reports "docs up to date"; msg-init's generated CLAUDE.md (read every session) misdirects the agent; plan-em emits wrong paths. **Fix:** one canonical path map in `shared/refs/`, referenced by every skill that touches these files. Impact High, effort Low.

**X4 — PRD content schema mismatch (plan-pm ↔ plan-tune ↔ plan-em).** plan-pm's PRD has no feature-spec table, no feature IDs, no acceptance criteria; plan-tune flags their absence as Critical on every clean PRD, and plan-em keys its execution table on feature IDs that are never written. **Fix:** add a required "Features & acceptance criteria" table to `template-prd.md` carrying F1/F2 IDs from the interview; re-calibrate plan-tune Dimensions 1–4 to the real 7-section template. Impact High, effort Medium. Clears defects in three skills at once.

**X7 — ship branch contract (ship ↔ eng).** ship creates `feat/prd-<n>-<slug>` and passes it to `eng --build` as `branch`, but eng treats `branch` as a PR target and cuts per-agent sub-branches. The feature branch gets no commits, and `/review`/`/test`/`/pre-merge` diff an empty branch. **Fix:** align the branch contract (build agents commit to the feature branch); add a post-build non-empty-diff gate. Impact High, effort Medium.
