...
**Safety floor.** The safety floor is **never relaxed**: write powers are scoped per skill (eng commits to feature branches only; pre-merge opens exactly one feature→staging PR and never merges; post-merge is the only merger, and nothing reaches `main` except via its double-confirmed staging→main release), and the human gates (preview approval, staging sign-off — pinned to the commit you tested, the inline human-test when shipping without staging, the always-ask rollback offer, production double-confirm) never disappear. `/post-merge --production` guards `main` behind branch protection (green CI + human review). See ARCHITECTURE.md § Safety floor.
