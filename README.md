...
| `/pre-merge` | The CI gate — takes a feature branch from "eng says done" to "PR open against staging": sync → mechanical → unit/int → regression → platform components → security/migration → PRD-consistency → preview deploy (human gate) → opens PR feature→staging. Absorbs the old `/review` and `/test`; emits a severity-graded JSON verdict. `--init` detects each step's tooling — including whether a `.github/workflows/` pipeline runs the gate on PRs — offers to install/scaffold the missing (free/OSS) pieces, and records the decisions in `devkit/policy.json`. (`--doctor` is a deprecated one-release alias for `--init`.) |
