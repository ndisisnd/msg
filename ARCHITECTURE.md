...
| `devkit/policy.json` | Committed, shared gate policy — release-flow shape (`staged`/`direct`), branch-protection stance, staging-readiness stance (`staging_readiness` mode + the per-platform `staging_ready` record `--init` resolves), and per-step tooling decisions. Read by both gates at run time; **decisions only** (never per-machine tool presence). Schema: `shared/refs/policy-schema.md` |
